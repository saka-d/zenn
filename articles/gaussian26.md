---
title: "Gaussian 26講習会 5日間レポート: 全体像 + 3テーマ深掘り"
emoji: "🧪"
type: "tech"
...
1. 公式ホームページPDF（募集要項・講習設計）
2. 公式日程表PDF（時刻つきタイムテーブル）
3. 当日メモ（`memo0216`〜`memo0220`）
4. 講習会報告スライド（`Gaussian講習会_坂口.pdf`）

今回は「広く浅く」だけで終わらないように、  
全体時系列を押さえたうえで次の3項目を深掘りします。
